...
bool GossipHello_custom_dualspec(Player* player, Creature* creature)
{
    ...
    {
        // Display cost
        const std::string costIs = player->GetSession()->GetMangosString(DUAL_SPEC_COST_IS) + std::to_string(uint32(sWorld.getConfig(CONFIG_UINT32_DUAL_SPEC_COST) / 10000)) + " g";
        player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, purchase, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 0, areYouSure, sWorld.getConfig(CONFIG_UINT32_DUAL_SPEC_COST), false);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, costIs, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 0);
    }
    else
    {
